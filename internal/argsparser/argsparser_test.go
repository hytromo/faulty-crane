...
func TestApply(t *testing.T) {
	...
	if cliOptions.Plan.SubcommandEnabled {
		t.Error("The plan command should not be enabled")
	}
...
func TestConfigure(t *testing.T) {
	cliOptions, err := Parse([]string{"app", "configure",
		"-out", "config.json",
	})
	...
	if cliOptions.Apply.SubcommandEnabled {
		t.Error("The apply command should not be enabled")
	}

	if !cliOptions.Configure.SubcommandEnabled {
		t.Error("The configure subcommand should be enabled")
	}
	...
	if cliOptions.Configure.Config != "config.json" {
		t.Error("Config should have a specific value")
	}
}

func TestNonArguments(t *testing.T) {
	_, err := Parse([]string{"app"})

	if err == nil {
		t.Error("Error should be reported on not enough arguments")
	}
}

func TestShow(t *testing.T) {
	cliOptions, err := Parse([]string{"app", "show",
		"-plan", "plan.out",
	})
	...
	if !cliOptions.Show.SubcommandEnabled {
		t.Error("The show command should be enabled")
	}

	if cliOptions.Show.Plan != "plan.out" {
		t.Error("Plan should have a specific value")
	}

	if cliOptions.Show.Analytical {
		t.Error("Analytical should be false")
	}

	os.Setenv(EnvPrefix+"ANALYTICAL", "true")
...
func TestWrongCommand(t *testing.T) {
	...
	if err == nil {
		t.Error("Invalid subcommand should error out")
	}
}
